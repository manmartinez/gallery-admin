 ...
 * inspired by https://github.com/rails/jquery-ujs
 *
 ...
 */
  ...
  $.laravel = laravel = {
    ...
    refreshCSRFTokens: function(){
      ...
      $('form input[name="_token"]').val(csrfToken);
    },
    ...
    handleRemote: function(element) {
      ...
      if (laravel.fire(element, 'ajax:before')) {
        ...
        } else if (element.is(laravel.buttonClickSelector)) {
        ...
        options = {
          ...
          beforeSend: function(xhr, settings) {
            ...
            if (laravel.fire(element, 'ajax:beforeSend', [xhr, settings])) {
  ...
  if (laravel.fire($document, 'laravel:attachBindings')) {
    ...
    $document.delegate(laravel.linkClickSelector, 'click.laravel', function(e) {
      ...
      if (!metaClick && link.is(laravel.linkDisableSelector)) laravel.disableElement(link);

      if (link.data('remote') !== undefined) {
        if (metaClick && (!method || method === 'GET') && !data) { return true; }

        var handleRemote = laravel.handleRemote(link);
        // response from laravel.handleRemote() will either be false or a deferred object promise.
        if (handleRemote === false) {
          laravel.enableElement(link);
        } else {
          handleRemote.error( function() { laravel.enableElement(link); } );
        }
        return false;
    ...
    $document.delegate(laravel.buttonClickSelector, 'click.laravel', function(e) {
      var button = $(this);
      if (!laravel.allowAction(button)) return laravel.stopEverything(e);

      laravel.handleRemote(button);
      return false;
    });
    ...
    $document.delegate(laravel.formSubmitSelector, 'submit.laravel', function(e) {
      ...
      if (form.attr('novalidate') == undefined) {
        ...
        if (blankRequiredInputs && laravel.fire(form, 'ajax:aborted:required', [blankRequiredInputs])) {
          return laravel.stopEverything(e);
        }
      }
      ...
      if (remote) {
        ...
        if (nonBlankFileInputs) {
          ...
          var aborted = laravel.fire(form, 'ajax:aborted:file', [nonBlankFileInputs]);

          // re-enable form elements if event bindings return false (canceling normal form submission)
          if (!aborted) { setTimeout(function(){ laravel.enableFormElements(form); }, 13); }
    ...
    $document.delegate(laravel.formSubmitSelector, 'ajax:send.laravel', function(event) {
